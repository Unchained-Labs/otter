...
5. Worker dequeues job, atomically claims next queued DB job, and marks it `running`.
6. Worker executes:
   - `vibe --prompt "<prompt>" --output json --workdir <workspace_root>`
   - with `VIBE_HOME=<isolated_workspace_home>`
7. Worker persists result and transitions status:
...
- API -> Client:
  - REST snapshots + live SSE events
...
If you want a simpler model, use `OTTER_DEFAULT_WORKSPACE_PATH` and omit `workspace_id` from the frontend; Otter will route all jobs through that default workspace context.
